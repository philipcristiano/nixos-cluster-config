
{ lib, config, pkgs, ... }:
let

  cfg = config.lab_radicle;
  name = "radicle";


in with lib; {
  options = {
    lab_radicle = {
      enable = mkOption {
        type = types.bool;
        default = true;
        description = ''
          Enable radicle?
        '';
      };
      expose_with_traefik = mkOption {
        type = types.bool;
        default = true;
        description = ''
          Enable exposing with traefik
        '';
      };
    };
  };
  config = mkIf config.lab_radicle.enable {
    services.radicle = {
        enable = true;
        privateKeyFile = config.sops.radicle_private_key.path;
        public_key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGHBUFatz3NJAtxCyV9TGVe8X6sSxP+hcb5DW2Iof00b";
        node.openFirewall = true;
        node.listenAddress = "0.0.0.0";
        node.listenPort = 8776;
    };

    services.radicle.settings = {


    };

    sops.secrets.radicle_private_key = {
          sopsFile = secrets/radicle.yaml;
          key = "private_key";
          mode = "400";
          restartUnits = ["radicle.service"];
    };
    sops.secrets.radicle_public_key = {
          sopsFile = secrets/radicle.yaml;
          key = "public_key";
          mode = "400";
          restartUnits = ["radicle.service"];
    };
    users.groups."${name}" = {};
    users.users."${name}" = {
      group = name;
      isSystemUser = true;
    };


    services.traefik.dynamicConfigOptions.http.routers.${name} = mkIf config.lab_radicle.expose_with_traefik {
        rule = "Host(`radicle.${config.homelab.domain}`)";
        service = "radicle@file";
    };
    services.traefik.dynamicConfigOptions.http.services.${name} = mkIf config.lab_radicle.expose_with_traefik {
      loadBalancer = {
        servers = [
          {
            url = "http://127.0.0.1:8776";
          }
        ];
      };
    };
  };
}
